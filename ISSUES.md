...
|---|---|---|
| [#128](https://github.com/confinia/ecobuilding/issues/128) Email via alert@confinia.io (Keycloak verify + Grafana alerts) | **Merged** (PR [#131](https://github.com/confinia/ecobuilding/pull/131)): Grafana contact point + "scrape target down" rule **live and verified** (provisioning loaded clean); `kc-smtp.sh` guarded by an SMTP pre-flight so `verifyEmail` only activates once creds work; realm bootstrap (`auth_stack/realm-confinia.json`) now carries the SMTP block sans secret. **Blocked: OVH rejects the password (535)** — and the stored value contains a space (malformed) + a fragment leaked to a local terminal → **rotate the mailbox password** | Operator: set a clean password (OVH manager + VM `secrets.env` line, single token) → `./deploy/deploy.sh` applies Keycloak + Grafana automatically |
