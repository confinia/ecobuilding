...
# 3. Exhausting the allowance must UPSELL (429 + a way out), never dead-end.
#    Generating 29 more fiches would take ~10 min, so the counter is set to the
#    limit directly (same store the API reads) and the REAL response asserted.
API_BASE="$API_BASE" TOKEN="$TOKEN" USAGE_FILE="$PWD/sandbox_stack/data/leads/usage.json" python3 <<'PY'
...
req = urllib.request.Request(
    f"{base}/api/v1/report/bdnb-bg-RPKN-5A8C-L5QV.pdf?lon=2.0815&lat=48.7020",
...
try:
    ...
    assert "Pro" in detail, detail                     # a way forward
    assert "contact@confinia.io" in detail, detail     # a way to reach a human
    print("   allowance exhausted -> 429 with upsell + support contact")
PY
echo "== signup journey OK (account autonomous: allowance, metering, upsell)"
